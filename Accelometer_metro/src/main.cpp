...
Seuraava koodi mittaa kiihtyvyyttä (liike) ja tulostaa sen arvon, jos se on yli 1.5 (testaa omalla laitteella mikä on sopiva) ja muuten tulostaa nollan. Lisää tästä koodista tarvittavat osat omaan koodiisi (tämä esimerkkikoodi löytyy myös lisämateriaalikansiosta nimelläLSM6DS3_esimerkki.zip). Laitteen pitäisi toimia siten, että aina kun laite tunnistaa liikkeen esimerkkikoodin arvoilla niin pilveen lähetetään arvo 1 ja levyllä oleva LED (pinni D6) sytytetään. Painonapilla (Button1 pinnissä D7) voidaan hälytys kuitata ja silloin lähetetään pilveen arvo 0 ja myös LED sammutetaan.
...
void httpRequest();
void liike();
void kuittaus();  //Kuittaus, kun painonappia painetaan.
...
uint16_t ir, full;
uint32_t luminosity;
volatile int mov1;  //Kun havaitun liikkeen arvo menee yli 1,5:den, saa tämä muuttuja jonkin arvon.
volatile int kuitattu = 1; //Kun havaitun liikkeen arvo menee yli 1,5:den, saa tämä muuttuja arvon 0. Sitten tieto lähetetään.
const int bushButtonPin = 7; //Painonapin pinni.
const int RedledPin = 5; //Punaisen ledin pinni. 
...
void setup() {
  pinMode(RedledPin, OUTPUT);  // sets the digital pin 7 as output
...
void liike(){
  ...
  if (IMU.accelerationAvailable()) {
    ...
    Serial.print(g);
    Serial.println("Liikutaan.");
    }
    else{
      Serial.print(0);  //muuten tulostetaan 0
      Serial.println("Ei liikettä.");
    }
