...
void laskeKeskiarvo(){
  ...
  for (i = 0; i < 10; i++)
  {
    ...
    //Serial.println("Arvot:"); //Näillä vain testataan arvojen mittaus ja keskiarvon laskeminen.
    //Serial.println(arvot[i]);
    summa = summa + arvot[i]; //Tallennetaan mitatut arvot.
    keskiarvo = summa / 10; //Keskiarvon laskeminen.      
  }
        
  Serial.println("Etäisyyden keskiarvo on: ");  //Keskiarvon tulostus.
  Serial.println(keskiarvo); 

  //delay(50000);  //Kuinka usein keskiarvo lasketaan. Tämä ei kuitenkaan ole välttämätön.
}